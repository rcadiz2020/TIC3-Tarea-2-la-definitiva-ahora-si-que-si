...
esp_err_t bmi270_init(i2c_master_bus_handle_t bus_handle) {
    // 1. Configuramos el dispositivo (dirección y velocidad)
    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = ESP_SLAVE_ADDR, // 0x68
        .scl_speed_hz = 100000,           // 100kHz (o lo que soporte el sensor)
    };

    // 2. Añadimos este dispositivo al bus existente
    ESP_RETURN_ON_ERROR(i2c_master_bus_add_device(bus_handle, &dev_cfg, &device), TAG, "Fallo al añadir BMI270 al I2C");

    // 3. Ejecutamos la secuencia de arranque
    softreset();
    chipid();
    initialization();
